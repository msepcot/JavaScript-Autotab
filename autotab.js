 ...
 *    by default the function sets up Numeric Autotabbing on all form input 
 *    fields that have the class 'autotab'.
 * 
 *  - Version: 0.2.0
 *    - Generic Autotab - Simulate a tab when the user has entered the maximum 
 *      allowable characters in a form input field.
 *    - Changes:
 *        + Refactored code to make a generic autotab function that accepts an 
 *          element and a regular expression formatted string. Attempts 
 *          autotab if String.Match( RegEx ) succeeds.
 *        + Added keyup_specialKey to check the keyCode value against known 
 *          special keys, we only want to attempt an autotab if the user makes 
 *          a change to the form field's value.
 *  - Version: 0.1.0
 *    - Numeric Autotab - Simulate a tab when the user has entered the 
 *      maximum allowable numeric characters in a form input field.
 */

/* 
 * doNumericAutotab( e )
 * 
 * This function changes form focus from the current form element to the next
 * form element when the value of the form field is a string of integers 
 * matching in length to the form field's maxLength value.
 * 
 ...
 * 
 * This function changes form focus from the current form element to the next 
 * form element when the value of the form field length matches the form 
 ...
 * autotab( elm, valid )
 * 
 * This function takes and element (elm) and a regular expression string
 * (valid) and performs a match on the element's value. If the match passes, 
 * we pass focus to the next form field.
 */
 ...
 * Special Keys as described in the Quirksmode article Javascript - Detecting 
 * keystrokes http://www.quirksmode.org/js/keys.html (as of 19 September 
 * 2007).
 */
 ...
 * 
 * Given a keyCode value, this function checks against the known keyCodes for 
 * Numeric Keys on both the keyboard and key pad.
 */
 ...
 * 
 * Given an event has fired, this function returns the Source Element or 
 * Target of the event.
 */
 ...
 * focusOn( elm )
 * 
 * Given an element, this function attempts to give that element the browser's 
 * focus.
 */
 ...
 * 
 * From: DHTML Utopia: Modern Web Design Using JavaScript & DOM, published by 
 * Sitepoint Scott Andrew's addEvent function, used to register events to an 
 * element through addEventListener or attachEvent.
 */
